 ...
 * 
 * @returns {object} returns object that is in preceptor docuement ex: {active: TRUE, firstName: "John", lastName: "Smith"} 
 * If no preceptor found then returns undefinded. 
 */
export function getParamedicPreceptor(employeeId: string) {
...
async function test(){
  await getAllActiveParamedicPreceptors(); 
}
